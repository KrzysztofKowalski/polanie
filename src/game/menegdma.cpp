...
int MENEGERDMA::HiInterrupt =
    0; // Jeden, gdy obslugiwane jest wyzsze przerwanie
...
MENEGERDMA::MENEGERDMA(int ile_MAX_chcesz_miec_melodii_WAV, int paragraphs) {
  ...
  Jest_SB = 0;
  if (alloc_DOS_memory((unsigned short)(paragraphs << 1), Segment_Address,
                       Selector)) {
    cprintf("\nBrak pamieci DPMI");
    exit(0);
  }
  ...
  if (ile_MAX_chcesz_miec_melodii_WAV)
    if (!Rezerwuj_Pamiec(
            ile_MAX_chcesz_miec_melodii_WAV)) // Zwraca ilosc obiektow OPERWAV
    {
...
/////////////////////////////////////////////////////////////////////////////
void MENEGERDMA::Init(int jest_SB, int irq, int port,
                      int kanal) { // 0x220     0x0083
...
MENEGERDMA::~MENEGERDMA(void) {
  ...
  if (OrginalnaKlawiatura)
    _dos_setvect(0x09, OrginalnaKlawiatura);
  if (OrginalTimer)
    _dos_setvect(0x08, OrginalTimer);
        ...
        default:  switch(key)
                   {
                       ...
                       case 157:if(right)pressedRCTR=0;   //Puszczenie CTR
                                 ...
                                 break;
                       case 70: case 44: case 46: if(pressedLCTR||pressedRCTR)
    old=0; //Wcisnieto 'z' lub 'c' gdy wcisniety jest lewy lub prawy CTR, to
    nie wprowadzaj tego znaku ('z' lub 'c') do bufora break;
                   }
                   prefix = 0;
                   break;
    }*/
...
void MENEGERDMA::daj_kanal(void) {
  ...
  case 0x0087:
    Rejest_adresowy_kanalu = 0;
    break;
  case 0x0083:
    Rejest_adresowy_kanalu = 2;
    break;
  case 0x0081:
    Rejest_adresowy_kanalu = 4;
    break;
  case 0x0082:
    Rejest_adresowy_kanalu = 6;
    break;
  default:
    Rejest_strony_kanalu = 0x0083;
    Rejest_adresowy_kanalu = 2;
...
int MENEGERDMA::Rezerwuj_Pamiec(int ile_chcesz_miec_melodii_DMA) {
  ...
  if (pamiec_rezerwowano)
    return max_ilosc_melodii_DMA;
  ...
  if (WAVy)
    ...
    max_ilosc_melodii_DMA = 0;
  if (max_ilosc_melodii_DMA)
    pamiec_rezerwowano = 1;
  return max_ilosc_melodii_DMA; // Boze! dzieki ze pamietales za mnie o tym
                                // +i...
}
...
int MENEGERDMA::InstalujWAV(char *nazwa) {
  ...
  rezultat_czytania =
      /*NowyWAV.*/ ZaladujWAV_z_dysku(nazwa, &WAVy[wolny_uchwyt]);
#ifdef SPYWAV
  f = fopen("raport.txt", "at");
  ...
  // 24.X.wolna_str[wolna_strona_fizyczna] = 0;  //Zwracamy rezerwacje - strona
  // wracado puli wolnych

  if (rezultat_czytania)
    return rezultat_czytania; // Jesli czytanie sie nie powiodlo zwroc
                              // przyczyne bledu
...
int MENEGERDMA::Zwolnij_uchwyt(int numer)
{
 ...
 if(WAVy[numer].jest_grany) return
PROBA_ZWOLNIENIA_UCHWYTU_OBECNIE_ODTWARZANEGO; if(!(WAVy[numer].zajety)) return
PROBA_ZWOLNIENIA_UCHWYTU_NULL;
...
int MENEGERDMA::Odtwarzaj(int Uchwyt) {
  /// 24.X.int wolna;
  if (!Jest_SB)
    return BRAK_SOUND_BLASTERA;
  if (Uchwyt >= max_ilosc_melodii_DMA)
    return PROBA_ODTWORZENIA_ZBYT_DUZEGO_UCHWYTU;
  if (jest_odtwarzany)
    return PROBA_ODTWORZENIA_PRZED_ZAKONCZENIEM_ODTWARZANIA;

  if (!(WAVy[Uchwyt].zajety))
    return PROBA_ODTWORZENIA_UCHWYTU_NULL;
  if (WAVy[Uchwyt].jest_grany)
    return PROBA_ODTWORZENIA_UCHWYTU_OBECNIE_ODTWARZANEGO;
...
int MENEGERDMA::Odtwarzaj_DMA() {
  ...
  if (((WAVy[odtwarzany_uchwyt].zagrano) + how_paragraphs) <
      (WAVy[odtwarzany_uchwyt].size)) {
    WAVy[odtwarzany_uchwyt].do_zagrania = how_paragraphs;
  } else {
    WAVy[odtwarzany_uchwyt].do_zagrania =
        (WAVy[odtwarzany_uchwyt].size) - (WAVy[odtwarzany_uchwyt].zagrano);
  }
  ...
  memcpy((void *)SegmentForSB,
         (void *)(((char *)(WAVy[odtwarzany_uchwyt].samples)) +
                  (WAVy[odtwarzany_uchwyt].zagrano)),
         WAVy[odtwarzany_uchwyt].do_zagrania);
  ...
  outp(Rejest_strony_kanalu,
       Po); // outportb(0x0003,VV); Jak milo, ze nie trzeba tego pchac
