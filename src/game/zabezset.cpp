/*ÉÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ»
  ş Plik  : ZABEZP.CPP                                                     ş
  ş Autor : Artur Bidzinski                                                ş
  ş Data  : 26 luty 1996r.                                                 ş
  ÇÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¶
  ş Opis  : To co utrudni - mam nadzieje - zycie tym co lamia              ş
  ČÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍĽ*/

#include "zabezset.h"

const long punkt_wejscia = 5392;
const char *koncowki[] = {"",   "01", "02", "03", "04", "05", "06",
...
unsigned int generuj_graf(unsigned int num) {
  ...
  s = ((unsigned long)31413 * (unsigned long)s + (unsigned long)13849) %
      (unsigned long)65536;

  return (unsigned int)(unsigned long)(((unsigned long)num * (unsigned long)s) /
                                       (unsigned long)65536);
}
...
int odswiez_Polan(int ktory) {
  ...
  lseek(plik, punkt_wejscia + (long)(ktory - 1) * sizeof(opis_klucza),
        SEEK_SET);
  read(plik, (void *)(&opis),
       sizeof(opis_klucza)); // Oczytanie klucza i miejsca odczytu w pliku zbior
