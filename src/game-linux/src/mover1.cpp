...
// PORT: kopia game/mover1.cpp (1:1, lustrzana - oryginal w game/ zostaje bez
// zmian) + trzy roznice naprawiajace blokade naprawy budynku przez kilku
// robotnikow wyslanych NARAZ, patrz raporty/naprawa-wielu-robotnikow.md:
//  - Fix A (Castle::Run, co==3): grupowy rozkaz naprawy (command==8) rozdaje
//    kazdemu robotnikowi (type==1) INNE pole nalezace temu samemu budynkowi
//    zamiast wspolnego punktu klikniecia lezacego WEWNATRZ budynku (w
//    oryginale wszyscy zbiegali na ten sam pierscien wolnych pol; pierwszy
//    stawal na cala naprawe, fala BFS reszty umierala na zapelnionym
//    pierscieniu i Move() kasowal rozkaz - robotnik stal bezczynnie na zawsze).
//  - Fix B (Mover1::Move): straznik rozkazu - rozkaz naprawy (8) nie jest
//    kasowany, gdy robotnik stoi zablokowany w tloku przy budynku.
//  - Fix C (Mover1::Repare, swiadome odstepstwo od DOS): robotnik staje,
//    gdy naprawiany budynek stoi (exist==1) i ma juz pelne hp, zamiast machac
//    ok. 8-9 s po ukonczeniu naprawy (licznik placeN rosl do 226, mimo ze
//    budynek przy hp==maxhp nie zbiera znacznikow).
// Zmiany oznaczone komentarzami "PORT: Fix A/B/C"; reszta pliku = oryginal.
...
/////////////////////////////////////////////////////////////////
void NewCow(int nr) {
  int i; // PORT: implicit int (Watcom) - dodane dla g++
...
void Mover1::Move() {
  ...
  if (place[x - dx][y - dy] != 0) {
    // PORT: Fix B - straznik rozkazu (raporty/naprawa-wielu-robotnikow.md).
    // W oryginale krok zablokowany + dystans <=1 od celu = commandN=0, czyli
    // bezglosne skasowanie ROZKAZU: robotnik wyslany do naprawy (8) stawal w
    // tloku przy budynku i juz nigdy do niej nie wracal (case 0 -> LookAround
    // zwraca 0 bez wroga). Rozkaz 8 ma teraz zostac: robotnik czeka / liczy
    // sciezke od nowa, zamiast porzucac naprawe.
    if ((type) && (command != 8) && (x - xe < 2) && (x - xe > -2) &&
        (y - ye < 2) && (y - ye > -2)) {
      commandN = 0;
    }
    if (!type) {
...
void Mover1::Repare() {
  inmove = 0;
  inattack = 0;
  // PORT: Fix C - rozwiaz budynek z pola celu (analogia do Pointer()/Who(),
  ...
  if (placeN[xe][ye] > 226 || placeG[xe][ye] == 47 ||
      ...
      // PORT: Fix C - odstepstwo od DOS: stop, gdy naprawiany budynek stoi
      // (exist==1) i ma pelne hp. W DOS jedyny wylacznik to licznik
      // placeN[cel]>226, a budynek zbiera znacznik tylko przy hp<maxhp -
      // po ukonczeniu naprawy robotnik machal jeszcze ok. 8-9 s bez efektu.
      // Straznik exist==1 chroni budowe (exist 3/4) i ruiny (bez zmian).
      (pol_b != NULL && pol_b->exist == 1 && pol_b->hp >= pol_b->maxhp)) {
    commandN = 0;
    placeN[xe][ye] = 1;
    delay = maxdelay;
    return;
...
void Building::Run() {
  ...
  if (dd) {
    ...
    if (IFF == 2) {
      int post; // PORT: implicit int (Watcom)
      for (post = 4; post < MaxUnitsInCastle; post++) {
...
void Castle::Run() {
  ...
  {
    int post, bud;
    // PORT: Fix A - rozdzielenie celu naprawy miedzy robotnikow grupy
    // (raporty/naprawa-wielu-robotnikow.md). Oryginal dawal KAZDEMU
    // wybranemu ten sam punkt (command.x, command.y) - klikniete pole
    // WEWNATRZ budynku, nieosiagalne: przy wysylce naraz wszyscy zbiegali na
    // ten sam pierscien wolnych pol wokol budynku, pierwszy stawal na cala
    // naprawe, fala BFS reszty umierala na zapelnionym pierscieniu, a Move()
    // kasowal rozkaz (commandN=0) - "blokuja sie i pracuje tylko jeden".
    // K-ty robotnik (type==1, licznik pol_k wspolny dla obu petli) dostaje
    // inne pole nalezace TEMU SAMEMU budynkowi: k==0 -> pole klikniecia
    // (bez zmian), k>0 -> kolejne z 7 pol 3x3 budynku poza srodkiem
    // (bx+1,by+1) i poza polem rodzenia krow (bx,by+2). Building::Run zbiera
    // znaczniki z wszystkich 9 pol (skan 3x3), wiec tempo naprawy jak w DOS;
    // grupa zachowuje sie jak wyslana osobno. Rozkazy 1 i 7 oraz cele inne
    // niz pole budynku (most place==2, palisada placeG==277) bez zmian.
    int pol_k = 0;     // licznik wybranych robotnikow (type==1)
    int pol_split = 0; // 1 = udalo sie rozwiazac budynek z pola klikniecia
    ...
    // PORT: 7 pol budynku do rozdania (wzgledem lewego gornego rogu bx,by):
    // caly 3x3 budynku bez srodka (bx+1,by+1) i bez pola rodzenia krow
    // (bx,by+2) - wejscie na pole krow konczy rozkaz przez Move (x==xe,y==ye)
    static const int POL_RING[7][2] = {{0, 0}, {1, 0}, {2, 0}, {0, 1},
                                       {2, 1}, {1, 2}, {2, 2}};
    for (post = 1; post < MaxUnitsInCastle; post++) {
      ...
      if (mm->wybrany) {
        ...
        if (command.command == 8 || command.command == 7 ||
            command.command == 1) {
          // PORT: Fix A - cel jednostki (domyslnie pole klikniecia)
          int pol_tx = command.x, pol_ty = command.y;
          if (pol_split && command.command == 8 && mm->type == 1) {
            if (pol_k > 0) {
              int pol_t = (pol_k - 1) % 7;
              pol_tx = pol_bx + POL_RING[pol_t][0];
              pol_ty = pol_by + POL_RING[pol_t][1];
            }
            pol_k++;
          }
          mm->SetEnd(pol_tx, pol_ty);
        }
        if (command.command == 2)
          mm->SetTarget(place[command.x][command.y]);
      }
    ...
    for (bud = 0; bud < MaxBuildings; bud++)
      for (post = 0; post < 6; post++) {
        ...
        if (mm->wybrany) {
          ...
          if (command.command == 8 || command.command == 7 ||
              command.command == 1) {
            // PORT: Fix A - jak wyzej, licznik pol_k wspolny dla obu petli
            int pol_tx = command.x, pol_ty = command.y;
            if (pol_split && command.command == 8 && mm->type == 1) {
              if (pol_k > 0) {
                int pol_t = (pol_k - 1) % 7;
