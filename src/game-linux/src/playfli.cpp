...
#include <stdlib.h>
#include <string.h>
#include "port.h" // PORT: warstwa SDL2 (ekran, paleta, czas)
...
// ZMIENNE GLOBALNE ----------------------------------------------------------
// PORT: ekran FLIC -> bufor SDL zamiast A0000; timing 8253 -> SDL_GetTicks (ms)
byte *screen;                            // bufor ekranu (ustawiany w play())
...
void init_timer() {} // PORT: timera sprzetowego nie programujemy

void deinit_timer() {}
long int read_timer() { return (long int)POL_GetTicks(); } // PORT: milisekundy
...
void wait_timer(int cframe) {
  ...
  do {
    t = read_timer();
    POL_Delay(1); // PORT: nie zapinamy CPU (oryginal czekal na licznik 8253)
  } while (t < te);
}
/////////////////////// play /////////////////////////
int play(char *s) {
  ...
  screen = (byte *)POL_FrameBuffer(); // PORT: cel animacji = bufor SDL
  source = (byte *)malloc(0xffff);
  FILE *fptr = fopen(s, "rb"); // PORT: NULL-guard zamiast *fopen()
  if (!fptr) {
    free(source);
    return 0;
  }
  ...
  if (ferror(&fh)) {
    ...
    if (status != 1) {
      ...
      delay_value = *_word;
      timer_delay = (delay_value / 70.0) * 1000.0; // PORT: milisekundy
      ...
      do {
        ...
        if (status != 1) {
          ...
          if (err) {
            ...
            if (pal_changed)
              move_palette();
            // PORT: klatka gotowa w buforze SDL - pokazujemy i czekamy
            POL_Present();
    ...
    fclose(fptr); // PORT: zamykamy oryginalny uchwyt (kopia FILE by byla UB)
    deinit_timer();
  }
  free(source);
  return (err);
...
void wait_sync() {
  // PORT: synchronizacji pionowej VGA nie symulujemy
}

// AKTUALIZACJA PALETY -------------------------------------------------------

void move_palette() {
  // PORT: paleta FLIC -> SDL (wartosci DAC VGA 0-63)
  POL_SetPalette(pal);
}
...
int check_escape() {
  // PORT: Esc przez zdarzenia SDL (zamiast odczytu portu 0x60)
  if (kbhit()) {
    int key = getch();
    if (key == 27)
      return 1;
